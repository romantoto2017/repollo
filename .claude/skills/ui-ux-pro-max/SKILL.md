...
Esta skill es una copia de `ui-ux-pro-max` v2.13.0, del repositorio
[nextlevelbuilder/ui-ux-pro-max-skill](https://github.com/nextlevelbuilder/ui-ux-pro-max-skill)
(licencia MIT, incluida en `LICENSE`).

Está vendorizada como skill de proyecto en vez de instalada como plugin porque
las sesiones de Claude Code en la web corren con `SKIP_PLUGIN_MARKETPLACE=true`
y no cargan plugins; las skills a nivel proyecto sí se cargan desde el repo.

Dos cambios respecto del original:
- Las rutas de los scripts usaban `${CLAUDE_PLUGIN_ROOT}`, que solo está
  definida cuando corre como plugin. Se reemplazaron por rutas relativas a la
  raíz del repositorio.
- Se excluyó `scripts/tests/` (320 KB, solo para desarrollo del plugin).

Para actualizar: volver a copiar desde el repositorio original y reaplicar
estos dos cambios. Si en cambio corrés Claude Code local, conviene instalarlo
como plugin y borrar esta carpeta:
