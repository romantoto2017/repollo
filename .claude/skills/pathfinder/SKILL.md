---
name: pathfinder
description: Map a codebase into feature-grouped flowcharts, identify duplicated concerns across features, and propose a unified architecture. Use when asked to "find the ideal path," unify duplicated systems, or audit architecture before a refactor. Emits a proposed unified flowchart plus per-system /writing-plans prompts.
---

# Pathfinder

You are an ORCHESTRATOR. Map the codebase into feature-grouped flowcharts, identify duplicated concerns, propose the simplest unified architecture, and hand off per-system plans to `/writing-plans`.
...
- `04-handoff-prompts.md` — copy-pasteable `/writing-plans` prompts per unified system
...
For each unified system in the proposal, write a ready-to-run `/writing-plans` prompt to `04-handoff-prompts.md`. Each prompt must:
...
Format each as a fenced code block the user can copy directly into `/writing-plans`.
...
- **Handoff, don't implement** — Pathfinder ends at plan prompts; `/writing-plans` and `/executing-plans` take it from there
